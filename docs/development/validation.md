...
65개 단위·컴포넌트 테스트와 1개 배포 JAR 통합 테스트: 총 **66개, 실패 0, 오류 0, 건너뜀 0**.
...
| CLI | 인자 오류, 완료 ID 정규화·범위, 경로 중첩 거부, 0행 교체, SELECT/CTAS 전체 변환 |
| 배포 JAR | 별도 Java 8 프로세스, 1만 DAG 변환과 재실행, 원본 SHA-256 일치, 실패 시 결과 바이트 보존 |
